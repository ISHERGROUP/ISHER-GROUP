...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(`${base}/storage/v1/object/sign/notes/${encodeURIComponent(product.file_path)}`, {
      method: 'POST',
      headers: { apikey: adminKey, Authorization: `Bearer ${adminKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ expiresIn: 600 })
    });
    if (!response.ok) return res.status(500).send('Unable to prepare download');
